...
void Delay::process()
{
    ...
    for(int i = 0; i < BUFFER_SIZE; i ++)
    {
        ...
        if(inputs_live[DELAY_SIGNAL])
        {
            ...
            circular_buffer[current_sample] *= input_floats[DELAY_FEEDBACK_AMOUNT];
...
void Delay::initialize_unique_graphics_objects()
{
    ...
    names = {name + " signal (input text box)", name + " delay time (input text box)",
             name + " feedback amount (input text box)", name + " wet/dry (input text box)"};
    ...
    text_colors = std::vector<SDL_Color *>(4, &color);
    prompt_texts = {"signal", "#", "# or signal", "# or signal"};
