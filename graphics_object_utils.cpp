    ...
    std::vector<std::string> texts, std::vector<Module *> parents)
{
    ...
    {
        button = new Button(names[i], locations[i], colors[i], text_colors[i], texts[i],
                            parents[i]);
    ...
    std::vector<bool> bs, std::vector<Module *> parents)
{
    ...
    for(unsigned int i = 0; i < names.size(); i ++)
    {
        toggle_button = new Toggle_Button(names[i], locations[i], colors[i],
                                          color_offs[i], text_color_ons[i],
                                          text_color_offs[i], fonts[i],
                                          text_ons[i], text_offs[i], bs[i],
                                          parents[i]);
    ...
    std::vector<std::vector<float> *> buffers)
{
    ...
    for(unsigned int i = 0; i < names.size(); i ++)
    {
        waveform = new Waveform(names[i], locations[i], colors[i], background_colors[i],
                                range_lows[i], range_highs[i], buffers[i]);
